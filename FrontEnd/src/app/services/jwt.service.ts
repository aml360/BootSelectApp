...
// TODO: Refactor this file, translating TSDoc to english
@Injectable({
	providedIn: 'root',
})
...
export class JwtService {
	...
	async refreshToken() {
		...
		const response: IAuthTokenRes = await this.httpClient
			.post<IAuthTokenRes>(`${env.API_URL}/jwtrefresh`, { tokenStr: this.token() })
			.toPromise();
	...
	rmToken(): void {
		cookieRm(env.JWT_NAME);
		localStorage.removeItem(env.JWT_NAME);
	}
	...
	updateJwt(token: string): void {
		localStorage.setItem(env.JWT_NAME, token);
		document.cookie = `${env.JWT_NAME}=${encodeURIComponent(token)}`;
	}

	/** Token codificado en base64 */
	private token = () => {
		const tkn = localStorage.getItem(env.JWT_NAME);
